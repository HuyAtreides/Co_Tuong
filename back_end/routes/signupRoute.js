...
const checkEmail = async (req, res, next) => {
  try {
    ...
    if (formData.changes && formData.changes['email'])
...
router.post('/', checkEmail, checkUsername, async (req, res) => {
  try {
    ...
    req.login(user, (err) => {
      if (err) return res.status(500).json({ user: user, message: err.message });
...
router.post('/settings', checkUsername, checkEmail, checkPassword, async (req, res) => {
  try {
